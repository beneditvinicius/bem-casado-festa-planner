...
export const createRibbonSlice: StateCreator<
  RootState,
  [],
  [],
  RibbonSlice
> = (set) => ({
  ribbonColors: [
    ...
    { id: '25', name: 'Preto (219)', code: '219', color: '#000000' },
  ],
  
  addRibbonColor: (color) => set((state) => ({
    ribbonColors: [...state.ribbonColors, color]
  })),
  
  removeRibbonColor: (id) => set((state) => ({ 
    ribbonColors: state.ribbonColors.filter(color => color.id !== id) 
  })),
