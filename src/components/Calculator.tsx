  ...
  return <Card className="w-full">
      <CardContent className="pt-6">
        ...
        <div className="space-y-6">
          ...
          return <div key={selection.id} className="p-4 border rounded-lg">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-medium">Sabor {index + 1}</h3>
                  {flavorSelections.length > 1 && <Button variant="ghost" size="icon" onClick={() => removeFlavorSelection(selection.id)} className="h-8 w-8 text-red-500">
                ...
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <Select value={selection.flavorId} onValueChange={value => handleFlavorChange(selection.id, value)}>
                      ...
                      <SelectContent>
                        {flavors.map(flavor => <SelectItem key={flavor.id} value={flavor.id} className="text-lg">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center">
                      <Button variant="outline" size="icon" onClick={() => decrementQuantity(selection.id)} className="rounded-r-none h-14 w-14" disabled={selection.quantity <= 0}>
                        <Minus className="h-5 w-5" />
                      </Button>
                      <Input id={`quantity-${selection.id}`} type="number" min="20" value={selection.quantity || ""} placeholder="Mínimo 20" onChange={e => handleQuantityChange(selection.id, e.target.value)} className="text-xl h-14 text-center rounded-none border-x-0" />
                      <Button variant="outline" size="icon" onClick={() => incrementQuantity(selection.id)} className="rounded-l-none h-14 w-14">
                  ...
                  {selectedFlavor && selection.quantity > 0 && <div className="text-sm text-muted-foreground">
          ...
          <Button variant="outline" onClick={addFlavorSelection} className="w-full flex items-center justify-center gap-2 h-12">
          ...
          <div className="flex flex-col md:flex-row justify-between gap-4">
            <Button variant="outline" onClick={handleReset} className="text-lg h-12">
              Limpar Tudo
            </Button>
            
            <Button variant="default" className="text-lg h-12 bg-[#eb6824] hover:bg-[#d25618]" onClick={() => scrollToSection('faq')}>
