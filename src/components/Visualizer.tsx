        ...
        <div className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Select value={selectedRibbonId} onValueChange={setSelectedRibbonId}>
                ...
                <SelectContent>
                  {ribbonColors.map(color => <SelectItem key={color.id} value={color.id}>
            ...
            <div>
              ...
              <Select value={selectedPackageId} onValueChange={setSelectedPackageId}>
                ...
                <SelectContent>
                  {packageColors.map(color => <SelectItem key={color.id} value={color.id}>
                      <div className="flex items-center">
                        <div className="w-4 h-4 rounded-full mr-2" style={{
                      backgroundColor: color.color,
                      border: color.color === '#FFFFFF' || color.color === '#F8F4E3' ? '1px solid #E2E8F0' : 'none'
          ...
          <div className="mt-8">
            <div className="aspect-video rounded-md overflow-hidden bg-muted flex items-center justify-center">
              {imageSrc ? <img src={imageSrc} alt="Visualização da combinação" className="w-full h-full object-cover" /> : <div className="text-muted-foreground text-center p-4">
                  <div className="w-56 h-32 mx-auto relative bg-white rounded shadow-md border">
                    ...
                    <div className="absolute inset-0 m-4 border" style={{
                  backgroundColor: packageColors.find(c => c.id === selectedPackageId)?.color || '#FFFFFF'
                }}></div>
                    <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-32 h-1.5" style={{
